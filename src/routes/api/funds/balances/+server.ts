...
export const GET: RequestHandler = async () => {
  ...
  const result = (funds as Array<{ id: string; name: string; targetCents: number | null }>).
    map((f) => {
      ...
      return { fundId: f.id, name: f.name, balanceCents: bal, targetCents: f.targetCents, percentToTarget: pctToTarget };
    });
