...
{#if !fund}
  <p>Loading…</p>
{:else}
  <div class="flex items-center justify-between mb-4 accent-l" style={`--accent:${fund.color ?? '#64748b'}`}>
    <h1 class="text-2xl font-bold break-words flex items-center gap-2">
  ...
  <div class="grid gap-4 mb-4" style="grid-template-columns: repeat(auto-fit,minmax(220px,1fr));">
    <div class="card accent-l" style={`--accent:${fund.color ?? '#64748b'}`}>
  ...
  {#if sparkPoints}
  <svg width={SPARK_W} height={SPARK_H} viewBox={`0 0 ${SPARK_W} ${SPARK_H}`} fill="none" xmlns="http://www.w3.org/2000/svg" style={`color:${fund.color ?? 'currentColor'}`}>
